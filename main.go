...
func (self *Game) Update() error {
	...
	if !self.HasFood {
		...
		for {
			...
			for _, part := range self.Player.Body {
				if part.X == float32(cx) && part.Y == float32(cy){ goto loop }
			}
			break
		}
	...
	if math.Abs(float64(self.Food.X - self.Player.Body[0].X)) < playerSize && 
		...
		if self.Player.Body[i].Direction == North { y += playerSize }
		if self.Player.Body[i].Direction == East { x -= playerSize }
		if self.Player.Body[i].Direction == West { x += playerSize }
		if self.Player.Body[i].Direction == South { y -= playerSize }
	...
	if self.Delay % (playerSize * 2) == 0 {
		for i := range self.Player.Body {
			...
			if self.Player.Body[i].PrevDirection == North { self.Player.Body[i].Y -= playerSize }
			if self.Player.Body[i].PrevDirection == East { self.Player.Body[i].X += playerSize }
			if self.Player.Body[i].PrevDirection == West { self.Player.Body[i].X -= playerSize }
			if self.Player.Body[i].PrevDirection == South { self.Player.Body[i].Y += playerSize }

			if self.Player.Body[i].X < self.PlayArea.Left { self.Player.Body[i].X = self.PlayArea.Right - playerSize }
			if self.Player.Body[i].X + playerSize > self.PlayArea.Right { self.Player.Body[i].X = self.PlayArea.Left }
			if self.Player.Body[i].Y < self.PlayArea.Top { self.Player.Body[i].Y = self.PlayArea.Bottom - playerSize }
			if self.Player.Body[i].Y + playerSize > self.PlayArea.Bottom { self.Player.Body[i].Y = self.PlayArea.Top }
		}
	}
